...
BiThrTree InorderThrfindb (BiThrTree p, DataType data);
BiThrTree InorderThrfindbjd (BiThrTree p);
...
void TraverserInOrderThreading (BiThrTree p) {
	...
	if (p) {
		...
		while (p) {
			printf("%c\n",p->data);
			p = InorderThrfindbjd (p);	//找到该节点的 后继节点
		}
	}
...
BiThrTree InorderThrfindbjd (BiThrTree p) {	
 ...
 /* 中序线索二叉树 查找前趋节点 ---- 从给定节点处开始 */
BiThrTree InorderThrfindp (BiThrTree p, DataType data) {
	/*找到线索二叉树的起始节点*/
	if (p) {
		while (p->ltag == 0) 
			p = p->lchild;
...
int main (void)	{
	...
	BiThrTree ret = NULL;
	char data = 'E';
	ret = InorderThrfindb (T, data);	
	printf ("B=ret->data:%c\n", ret->data);
	
	ret = InorderThrfindp (T, data);
	printf ("P=ret->data:%c\n", ret->data);
